...
--------------------------------
Implements STRIPS-style planning using A*.
"""
...
def forward_search(initial_state, goal_state, actions):
    ...
    def heuristic(state):
        # Estimate remaining travel cost based on distances.
        remaining_cost = 0
        for customer, cost in customer_costs.items():
            if f'delivered_{customer}' not in state:
                remaining_cost += cost

        # Add return-to-base requirement if needed.
        if 'at_base' in goal_state and 'at_base' not in state:
            remaining_cost += base_pickup_cost

        # Penalize low battery when far from base.
        distance_to_base = 0
        if 'at_pickup' in state:
            distance_to_base = base_pickup_cost
        else:
            for customer, cost in customer_costs.items():
                if f'at_{customer}' in state:
                    distance_to_base = cost
                    break

        battery = battery_level(state)
        if battery < distance_to_base:
            remaining_cost += (distance_to_base - battery)
        ...
        for action in actions:
            if is_applicable(state, action):
                ...
                if new_state not in best_g or new_g < best_g[new_state]:
                    ...
                    heapq.heappush(
                        frontier, (f_cost, new_g, new_state, plan + [action['name']])
                    )
